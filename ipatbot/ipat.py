...
def login(userid: int, password: int, pars: int) -> requests.Response:
    ...
    soup = bs4.BeautifulSoup(response.content.decode("euc-jp"))
...
def bet(response: requests.Response, codes: typing.List[str], total_amount: int):
    # 馬券コード入力
...
def bet_codes(
    ...
    response = bet(codes, total_amount)
